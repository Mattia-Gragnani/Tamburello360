...
function buildEnrichedMatches(matchEntries, teamLookup) {
  ...
  const recommendedBySerie = enriched.reduce((acc, match) => {
    const serie = match.serie || 'A';
    if (!acc[serie]) acc[serie] = [];
    acc[serie].push(match);
    return acc;
  }, {});

  const recommendedRounds = Object.entries(recommendedBySerie).reduce((acc, [serie, serieMatches]) => {
    acc[serie] = getRecommendedGiornata(serieMatches);
    return acc;
  }, {});

  return enriched.map(m => ({
    ...m,
    is_recommended: m.giornata === recommendedRounds[m.serie || 'A']
  }));
}
